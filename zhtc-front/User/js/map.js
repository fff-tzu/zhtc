...
var bottom_right_control = new BMap.ScaleControl({ anchor: BMAP_ANCHOR_BOTTOM_RIGHT, offset: new BMap.Size(0, 30) }); //比例尺
...
function drawBoundary() {
    ...
    for (var i = 0; i < blist.length; i++) {
        ...
        //添加多边形层并显示
        var ply = new BMap.Polygon(blist[i].points, { strokeWeight: 5, strokeColor: "#FFFFFF", fillOpacity: 0.01, fillColor: "#fffff" }); //建立多边形覆盖物
    ...
    //添加遮蔽层
    // var plyall = new BMap.Polygon(pArray, { strokeOpacity:1, strokeColor: "#FF0000", strokeWeight: 0.00001, fillColor: "#FF0000", fillOpacity: 1 }); //建立多边形覆盖物
...
var wsmap = {
    ...
    mapLoad: function () {
        this.map = this.map
        this.map.centerAndZoom(new BMap.Point(120.1551, 30.2741), 15);
        ...
        var bottom_left_control = new BMap.ScaleControl({ anchor: BMAP_ANCHOR_BOTTOM_RIGHT, offset: new BMap.Size(0, 30) }); //比例尺
    ...
    showPolyline: function () {
        ...
        var startX = 119.9466; // 起始经度
        var endX = 120.3655; // 结束经度
        var startY = 30.1315; // 起始纬度
        var endY = 30.4161; // 结束纬度

        var gridWidth = (endX - startX) / 200; // 网格宽度
        var gridHeight = (endY - startY) / 200; // 网格高度

        for (var i = 0; i <= 200; i++) {
            var x = startX + gridWidth * i;
            var polyline = new BMap.Polyline([
                new BMap.Point(x, startY),
                new BMap.Point(x, endY)
            ], { strokeColor: "black", strokeWeight: 1, strokeOpacity: 0.5 });
            this.map.addOverlay(polyline);
        }

        for (var i = 0; i <= 100; i++) {
            var y = startY + gridHeight * i;
            var polyline = new BMap.Polyline([
                new BMap.Point(startX, y),
                new BMap.Point(endX, y)
            ], { strokeColor: "black", strokeWeight: 1, strokeOpacity: 0.5 });
            this.map.addOverlay(polyline);
        }
    }
